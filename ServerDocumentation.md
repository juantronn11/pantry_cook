# Pantry Cook
#### Document Owned by: Tina Carter

## Table of Contents
* [References Used](#references-used)
* [Current State of the Server](#current-state-of-the-server)
* [How to Duplicate this Portion of the Project](#how_to_duplicate_this_portion_of_the_project)
* [How to Update the Global Page](#how-to-update-the-global-page)

## References Used
`Ubuntu XX.X` `Apache 2.4.43`

<br>&emsp; "," , .

## Current State of the Server 
`private`

Imediate next step: run through a port so it can be accessed through the network.

Next Steps: 
* import PantryCook to server rather than test website
* get a domain name and connect to internet
* check for continuted functionality of website (when not run through terminal/as https://localhost:5173/ etc.)

## How to Duplicate this Portion of the Project

words

## How to Update the Global Page

1. what to do to update the server
2. and then this.

## Project By:
Miguel Alvarez, Tina Carter, Juan Estrada, Christian Johnson, Patrick Rucker