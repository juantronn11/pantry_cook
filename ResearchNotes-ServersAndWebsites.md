...
- router (not the modem, lol I spent way too long looking for the router password on my modem. Please get eight hours of sleep and maybe google what it looks like after 5 minutes unstead of 20).<br>&emsp;"A modem connects your home to your Internet Service Provider (ISP), while a router connects your devices to each other and to the modem's internet signal" more importanty router is the tall square base white tower, modem is the black toaster looking thing. I definitely knew that at 10:00, but 02:00 less so.<br>&emsp;router = how network traffic is directed (lol, imagine that, a router routes things, ten points for captain obvious). this is why we take system security. IP = name of your device's network connection, port = outward facing way to access a device OR a network. both should be protected, but at various levels... like ports should verify accessors, but do exist to be accessed, IP addressed are how you access things/verify 'yourself', but also not the best way to access a website (i.e. other people shouldn't be typing in an IP address of the server where the website is hosted, for conveniance, but also to cover the IP address to avoid having bad things done to/with it)
...
<br>&emsp; "Modem vs Router: What's the Difference?," xfinity, https://www.xfinity.com/hub/internet/modem-vs-router
